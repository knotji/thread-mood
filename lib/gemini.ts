...
export async function pickPhotosWithGemini({
  ...
  // Map all image parts for Gemini
  ...
  // STAGE 1: Shortlist selection
  const shortlistPrompt = `
คุณคือผู้ช่วยเลือกรูปภาพระดับมืออาชีพสำหรับลงโซเชียลมีเดียในภาษาไทย

วิเคราะห์รูปภาพทั้ง ${images.length} รูปที่ส่งมาให้ ซึ่งมีหมายเลขรูปดังนี้:
${images.map((img) => `- รูปภาพที่ส่งมามีหมายเลขประจำตัวเดิมคือ: "${img.originalIndex}" (ตรงกับ "รูป ${img.originalIndex}" ที่ผู้ใช้เห็น)`).join("\n")}

เป้าหมายการลง:
- แพลตฟอร์ม/การใช้งาน: ${platform}
- Mood/โทนที่ต้องการ: ${mood}

ให้ประเมินรูปภาพทั้งหมดและเลือกคัดสรรรูปภาพที่ดีที่สุดและเข้าพวกกันที่สุดจำนวน 2 ถึง 6 รูปเพื่อเข้ารอบ (shortlist) สำหรับนำไปใช้งานและวิเคราะห์ในขั้นตอนถัดไป

ให้ผลลัพธ์เป็น JSON ตาม schema นี้เท่านั้น:
- shortlistedIndices: อาร์เรย์ของหมายเลขรูปภาพต้นฉบับที่ได้รับการคัดเลือกให้เข้ารอบ (ต้องเป็นตัวเลขจากรายการนี้เท่านั้น: [${images.map(img => img.originalIndex).join(", ")}]) และต้องมีความยาวระหว่าง 2 ถึง 6 ตัวเลข ห้ามใส่ตัวเลขอื่นนอกเหนือจากนี้เด็ดขาด

ตอบเป็น JSON เท่านั้น
`;

  let shortlistedIndices: number[] = [];
  try {
    const shortlistResponse = await generateContentWithModelFallback(ai, {
      contents: [
        {
          role: "user",
          parts: [
            { text: shortlistPrompt },
            ...allImageParts,
          ],
        },
      ],
      config: {
        responseMimeType: "application/json",
        responseSchema: shortlistResponseSchema,
        temperature: 0.4,
      },
    });

    const shortlistData = parseJson(shortlistResponse.text ?? "") as any;
    if (shortlistData && Array.isArray(shortlistData.shortlistedIndices)) {
      shortlistedIndices = shortlistData.shortlistedIndices
        .map(Number)
        .filter((n: number) => !isNaN(n) && images.some((img) => img.originalIndex === n));
    }
  } catch (err) {
    console.error("Stage 1 Shortlist selection failed", err);
  }

  // Fallback shortlist if Stage 1 didn't return valid indices (2 to 6 indices)
  if (shortlistedIndices.length < 2 || shortlistedIndices.length > 6) {
    shortlistedIndices = images.slice(0, 6).map((img) => img.originalIndex);
  }

  // Filter to only shortlisted images
  const shortlistedImages = images.filter((img) => shortlistedIndices.includes(img.originalIndex));

  const detailedPrompt = `
คุณคือผู้ช่วยเลือกและจัดระเบียบรูปภาพสำหรับลงโซเชียลมีเดียในภาษาไทย

คุณได้รับคัดเลือกรูปภาพจำนวน ${shortlistedImages.length} รูปนี้จากทั้งหมด ${images.length} รูปที่ส่งมาในขั้นตอนแรก
โดยผู้ใช้ระบุป้ายกำกับของรูปภาพแต่ละรูปที่ผ่านเข้ารอบดังนี้:
${shortlistedImages.map((img, idx) => `- รูปที่ ${idx + 1} ในเซตนี้ตรงกับหมายเลข "${img.originalIndex}" (ผู้ใช้เห็นเป็น "รูป ${img.originalIndex}")`).join("\n")}
...
- moodSummary: คำอธิบายสรุปบรรยากาศรวมของเซตรูปภาพนี้ (ภาษาไทย) และเขียนระบุด้วยว่าคัดเลือกจากรูปภาพทั้งหมด ${images.length} รูป เหลือรูปที่คัดสรรแล้ว ${shortlistedImages.length} รูป
- bestPhotoIndex: หมายเลขรูปภาพที่ดีที่สุด (ต้องเป็นตัวเลข original index ตัวใดตัวหนึ่งในเซตนี้เท่านั้น: [${shortlistedImages.map(img => img.originalIndex).join(", ")}])
- bestPhotoReason: เหตุผลภาษาไทยที่รูปนี้เหมาะกับแพลตฟอร์มและ mood ที่ต้องการมากที่สุด
- rankedPhotos: จัดอันดับรูปภาพที่เด่นที่สุด 3 อันดับแรก (มี index (ต้องเป็นหนึ่งในตัวเลข original index: [${shortlistedImages.map(img => img.originalIndex).join(", ")}]), score (คะแนน 1-10), suggestedUse (คำแนะนำในการนำไปใช้เป็นภาษาไทย))
- skipPhotos: รูปที่แนะนำให้ข้าม (มี index (ต้องเป็นหนึ่งในตัวเลข original index จากเซตนี้: [${shortlistedImages.map(img => img.originalIndex).join(", ")}]) และ reason ภาษาไทย)
...
- feedOrder: (เฉพาะถ้าแพลตฟอร์มคือ Feed) ลำดับรูปที่ควรจัดเรียงใน Carousel/Swipe Feed (ต้องเป็นตัวเลข original index เช่น [${shortlistedImages.map(img => img.originalIndex).join(", ")}])
...
- การระบุหมายเลขรูปภาพ (index, bestPhotoIndex, rankedPhotos.index, skipPhotos.index, storySequence.index, feedOrder, coverIndex) ต้องใช้ตัวเลข original index จากเซตนี้เท่านั้น: [${shortlistedImages.map(img => img.originalIndex).join(", ")}] ห้ามใช้เลขอื่นเด็ดขาด!
- ให้ผลลัพธ์เป็น JSON ตรงตาม schema เท่านั้น ห้ามมี markdown บล็อกครอบนอก JSON

ตอบเป็น JSON เท่านั้น
`;

  const shortlistedImageParts = shortlistedImages.map((img) => ({
    inlineData: {
      data: img.base64,
      mimeType: img.mimeType,
    },
  ...
  const detailedResult = normalizePhotoPickResult(parseJson(response.text ?? ""), shortlistedIndices, platform, mood);
  return {
    ...detailedResult,
    totalUploaded: images.length,
    shortlistedCount: shortlistedIndices.length,
